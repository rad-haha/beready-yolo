...
import warnings
from fastapi import FastAPI, APIRouter  # REST API 서버 프레임워크
...
def detect_people(camera_index, video_path):  # 사람 탐지 함수
    ...
    while True:  # 무한 루프 시작
        ...
        results = model(frame, conf=0.2, iou=0.5, max_det=20, verbose=False)[
            0]  # 프레임을 YOLO 모델에 입력하고, 첫 번째 결과 ([0])를 가져옴
